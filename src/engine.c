            ...
            if (g_use_mtp && g_e3.loaded) {
                ...
                if (li == E3_TARGET_LAYERS[2]) {
                    ...
                    if (e3_captured == 3) {
                        ...
                        int e3_pred = e3_forward(e3_h0, e3_h1, e3_h2, prompt[i], i, e3_logits, c, m);
                        int expected = (i + 1 < np) ? prompt[i + 1] : -1;
                        fprintf(stderr, "[e3prefill] pos=%d pred=%d expected(i+1)=%d match=%d\n", i, e3_pred, expected, e3_pred == expected);
                        free(e3_logits);
                    }
                }
            }
