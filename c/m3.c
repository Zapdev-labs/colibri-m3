/* MiniMax-M3 streaming MoE inference engine for Colibri.
 * GQA attention, per-head QK RMSNorm, partial RoPE, SwiGLU-OAI.
 * Expert weights streamed from disk (int8/int4 per-row quant, dequant-on-use).
 * MoE router: sigmoid + e_score_correction_bias + top-k (routed_scaling_factor).
 * Optional PLANAR_KV=1: PlanarQuant-style 2D Givens + int3/4 KV cache.
 * AVX-512 kernels when compiled with -mavx512f (Xeon Gold 5220R etc).
 *
 * Build: make m3
 * Run:   SNAP=/path/to/m3_i4 ./m3 <cap> <expert_bits> <dense_bits>
 */
#define _GNU_SOURCE
#include <stdio.h>
#include <stdlib.h>
...
static inline float dot_avx512(const float *a, const float *b, int n){
...
static inline void axpy_avx512(float *y, const float *x, float w, int n){
    __m512 wv=_mm512_set1_ps(w); int i=0;
    for(;i+16<=n;i+=16){
...
#define dot_fast dot_avx512
#define axpy_fast axpy_avx512
#else
#define dot_fast dot_avx2
#define axpy_fast axpy_avx2
#endif
...
typedef struct {
    ...
    int route_norm, rotary_dim, gemma_norm;
    float eps, theta, router_scale, swiglu_alpha, swiglu_limit;
...
static float g_sw_alpha=1.702f, g_sw_limit=7.f;
static int g_planar_kv=0, g_planar_bits=3;
static int g_topk=0;
...
static void rmsnorm_head(float *out, const float *x, const float *w, int hd, float eps){
    ...
    if(g_gemma_norm) for(int i=0;i<hd;i++) out[i]=x[i]*r*(1.f+w[i]);
...
static inline float swiglu_oai(float x, float alpha, float limit){
    float xg = x > limit ? limit : (x < -limit ? -limit : x);
    return xg * sigmoidf(alpha * xg);
}
...
/* Partial rotate_half RoPE: only first rotary_dim channels (MiniMax-M3). */
static void rope_rotate_half(float *x, int pos, float theta, int hd, int rotary_dim){
    int rd=rotary_dim>0&&rotary_dim<=hd?rotary_dim:hd;
    int h2=rd/2; float tmp[512];
    if((size_t)rd>sizeof(tmp)/sizeof(float)){ fprintf(stderr,"rotary_dim too large\n"); exit(1); }
    memcpy(tmp,x,(size_t)rd*sizeof(float));
    for(int j=0;j<h2;j++){
        float inv=powf(theta,-2.0f*j/(float)rd), ang=pos*inv, c=cosf(ang), s=sinf(ang);
...
static void load_cfg(Cfg *c, const char *snap){
    char *ar=NULL; jval *root=cfg_root(snap,&ar);
    ...
    c->n_experts=gi(r,"num_experts"); if(!c->n_experts) c->n_experts=gi(r,"num_local_experts");
    ...
    if(!c->first_dense){
        jval *freq=json_get(r,"moe_layer_freq");
        if(freq&&freq->t==J_ARR){ int z=0; for(int i=0;i<freq->len;i++){ if((int)freq->kids[i]->num) break; z++; } c->first_dense=z; }
    }
    c->head_dim=gi(r,"head_dim"); if(!c->head_dim) c->head_dim=c->hidden/c->n_heads;
    c->n_shared=gi(r,"num_shared_experts"); if(!c->n_shared) c->n_shared=gi(r,"n_shared_experts");
    ...
    c->theta=th?(float)th->num:5000000.f;
    c->rotary_dim=gi(r,"rotary_dim"); if(!c->rotary_dim) c->rotary_dim=c->head_dim/2;
    c->swiglu_alpha=gf(r,"swiglu_alpha",1.702f);
    c->swiglu_limit=gf(r,"swiglu_limit",7.f);
    jval *gn=json_get(r,"use_gemma_norm"); c->gemma_norm=(gn&&gn->t==J_BOOL)?gn->boolean:1;
    g_sw_alpha=c->swiglu_alpha; g_sw_limit=c->swiglu_limit; g_gemma_norm=c->gemma_norm;
    c->n_stop=0;
    jval *eo=json_get(r,"eos_token_id"); if(!eo) eo=json_get(root,"eos_token_id");
...
static void model_init(Model *m, const char *snap, int cap, int ebits, int dbits){
    ...
    /* testa MTP (layer n_layers): presente solo se convertita con --mtp */
    {
        ...
        /* MiniMax-M3 source naming: shared_mlp / final_layernorm; Colibri rename: shared_experts / shared_head.norm */
        if(m->has_mtp){
...
static void attention(Model *m, Layer *l, int layer, float *x, int S, int pos_base, float *out){
    ...
    for(int s=0;s<S;s++){
        ...
        for(int h=0;h<H;h++){
            ...
            rope_rotate_half(qh,pos,c->theta,hd,c->rotary_dim);
        }
        for(int kh=0;kh<Hkv;kh++){
            ...
            rope_rotate_half(kk,pos,c->theta,hd,c->rotary_dim);
            if(g_kv_i8){
                int64_t off=((int64_t)kh*m->max_t+pos)*hd;
                int64_t soff=(int64_t)kh*m->max_t+pos;
                if(g_planar_kv){
                    m->Kscale[layer][soff]=planar_kv_encode(kk,m->Kq[layer]+off,hd,g_planar_bits);
                    m->Vscale[layer][soff]=planar_kv_encode(vv,m->Vq[layer]+off,hd,g_planar_bits);
                } else {
        ...
        for(int jj=0;jj<nt;jj++){
            ...
            if(g_kv_i8){
                if(t==pos){
                    ...
                    sc[nv]=dot_fast(qv,kv,hd)*scale;
                } else {
                    int64_t off=((int64_t)kvh*m->max_t+t)*hd;
                    int64_t soff=(int64_t)kvh*m->max_t+t;
                    if(g_planar_kv) sc[nv]=planar_kv_dot_q(qv,m->Kq[layer]+off,m->Kscale[layer][soff],hd,g_planar_bits)*scale;
                    else sc[nv]=m->Kscale[layer][soff]*dot_f_i8(qv,m->Kq[layer]+off,hd)*scale;
                }
            } else {
        ...
        for(int jj=0;jj<nt;jj++){
            ...
            if(g_kv_i8){
                if(t==pos){
                    ...
                    axpy_fast(cx,vv,sc[vi],hd);
                } else {
                    int64_t off=((int64_t)kvh*m->max_t+t)*hd;
                    int64_t soff=(int64_t)kvh*m->max_t+t;
                    if(g_planar_kv){
                        float tmpv[512];
                        if(hd>512){ fprintf(stderr,"head_dim too large for planar tmp\n"); exit(1); }
                        planar_kv_decode(tmpv,m->Vq[layer]+off,m->Vscale[layer][soff],hd,g_planar_bits);
                        axpy_fast(cx,tmpv,sc[vi],hd);
                    } else axpy_f_i8(cx,m->Vq[layer]+off,sc[vi]*m->Vscale[layer][soff],hd);
                }
            } else {
...
static void dense_mlp(Layer *l, float *x, int S, int D, int I, float *out){
    ...
    for(int64_t i=0;i<(int64_t)S*I;i++) g[i]=swiglu_oai(g[i],g_sw_alpha,g_sw_limit)*u[i];
    matmul_qt(out,g,&l->down_proj,S);
    free(g); free(u);
}

/* MoE: MiniMaxM3TopKRouter math (sigmoid, bias for selection, normalize, router_scaling_factor) */
static void moe(Model *m, Layer *l, int layer, float *x, int S, float *out){
    ...
    for(int base=0;base<nu;base+=64){
        ...
        for(int j=0;j<nb;j++){ int eid=uniq[base+j]; ESlot *e=use[j];
            ...
            for(int64_t z=0;z<(int64_t)nr*I;z++) gg[z]=swiglu_oai(gg[z],g_sw_alpha,g_sw_limit)*uu[z];
    ...
    for(int64_t z=0;z<(int64_t)S*sI;z++) sg[z]=swiglu_oai(sg[z],g_sw_alpha,g_sw_limit)*su[z];
...
static const char *m3_effort(int think){
...
static int m3_wrap(char *buf, int cap, const char *user, int think){
    const char *eff=m3_effort(think);
...
static void run_text(Model *m, const char *snap, const char *prompt, int ngen){
    ...
    char wrapped[1<<16]; int bl=m3_wrap(wrapped,sizeof(wrapped),prompt,think);
...
static void run_serve(Model *m, const char *snap){
    ...
    while((nr=getline(&line,&cap,stdin))>0){
        ...
        if(raw_mode){
            ...
            if(templ) bl=m3_wrap(buf,1<<16,input,think);
            ...
            if(len+k+req_ngen+2>=maxctx){ len=0; first=1; kv_alloc(m,4096);
                if(m->has_mtp) m->kv_start[m->c.n_layers]=-1;
                bl=0; if(templ) bl=m3_wrap(buf,1<<16,input,think);
                else bl=snprintf(buf,1<<16,"%s",input);
...
int main(int argc, char **argv){
    ...
    { const char *pk=getenv("PLANAR_KV"); if(pk&&*pk&&*pk!='0'){ g_planar_kv=1; g_kv_i8=1; }
      const char *pb=getenv("PLANAR_BITS"); if(pb){ int b=atoi(pb); if(b==3||b==4) g_planar_bits=b; } }
    ...
    if((getenv("COLI_CUDA") && atoi(getenv("COLI_CUDA"))) ||
       ...
       (getenv("CUDA_EXPERT_GB") && atof(getenv("CUDA_EXPERT_GB"))>0)){
        fprintf(stderr,"CUDA was requested, but this binary is CPU-only; rebuild with: make m3 CUDA=1\n");
        return 2;
    }
#endif
    printf("== MiniMax-M3 C engine, cache=%d experts/layer | experts@%d-bit dense@%d-bit ==\n",cap,ebits,dbits);
    ...
    const char *refpath=getenv("REF")?getenv("REF"):"ref_m3.json";
    ...
    if(getenv("TF")){
        ...
        { int maxid=0; for(int i=0;i<nfull;i++) if(full[i]>maxid) maxid=full[i];
          if(m.c.vocab>1000&&maxid<1000&&!getenv("REF_FORCE")){
            fprintf(stderr,"ERRORE: ref_m3.json is the tiny oracle (max id %d, vocab %d).\n"
                           "  Self-test: SNAP=./m3_tiny TF=1 ./m3 64 16 16\n",maxid,m.c.vocab);
            return 1;
          } }
    ...
    printf("\nMiniMax-M3 C engine: "); for(int i=np;i<nfull;i++){ printf("%d ",out[i]); if(out[i]==full[i]) match++; }
    printf("\nMatching tokens: %d/%d\n",match,n_new);
    double tot=m.hits+m.miss;
